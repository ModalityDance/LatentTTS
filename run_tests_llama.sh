...
typeset -a DROPOUT_PS=(0.03)
# typeset -a NOISE_STDS=(0.01)
...
# Function to run llama inference and capture results
run_llama_inference() {
    local split=$1
    ...
    for seed in $SEEDs; do
        ...
        for i in {1..${#Ns}}; do
            ...
            if [[ $sampling_method == "dropout" ]]; then
                ...
                # Extract more detailed results for llama
                ...
                python -m src.infer_llama \
                    ...
                    --n_samples=$N 2>&1 | tee $temp_file
                # Extract standard results for gpt2
                local results=$(grep -E "$grep_template" $temp_file)
            fi
...
for split in $SPLITS; do
    for dropout_p in $DROPOUT_PS; do
        run_llama_inference $split "dropout" "dropout" $dropout_p
    done
done
...
for split in $SPLITS; do
    for noise_std in $NOISE_STDS; do
        run_llama_inference $split "noise" "noise" $noise_std
    done
done
